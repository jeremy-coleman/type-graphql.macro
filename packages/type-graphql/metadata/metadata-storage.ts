...
export class MetadataStorage {
  ...
  private buildFieldResolverMetadata(
    ...
    definitions.forEach(def => {
      ...
      def.getObjectType =
        def.kind === "external"
          ? this.resolverClasses.find(resolver => resolver.target === def.target)!
              .getObjectType
          : () => def.target as ClassType
      if (def.kind === "external") {
        const typeClass = this.resolverClasses.find(
          resolver => resolver.target === def.target
        )!.getObjectType!()
        const typeMetadata =
          this.objectTypes.find(objTypeDef => objTypeDef.target === typeClass) ||
          this.interfaceTypes.find(
            interfaceTypeDef => interfaceTypeDef.target === typeClass
          )
        if (!typeMetadata) {
          throw new Error(
        ...
        if (!typeField) {
          const shouldCollectFieldMetadata =
            ...
            options.resolvers.some(
              resolverCls =>
                resolverCls === def.target || def.target.isPrototypeOf(resolverCls)
            )
    ...
    this.resolverClasses.forEach(def => {
      ...
      while (superResolver.prototype) {
        ...
        if (superResolverMetadata) {
          ...
          this.subscriptions = mapSuperResolverHandlers(
            this.subscriptions,
            superResolver,
            def
          )
  ...
  private findExtensions(target: Function, fieldName?: string): ExtensionsMetadata {
    const storedExtensions: Array<ExtensionsClassMetadata | ExtensionsFieldMetadata> =
      fieldName ? this.fieldExtensions : this.classExtensions

...
export abstract class SchemaGenerator {
  ...
  static async generateFromMetadata(
  ...
  private static checkForErrors(options: SchemaGeneratorOptions) {
    // type-graphql.macro: this is now checked at compile-time
    // ensureInstalledCorrectGraphQLPackage();
    if (
      getMetadataStorage().authorizedFields.length !== 0 &&
  ...
  private static buildTypesInfo(resolvers?: Function[]) {
    this.unionTypesInfo = getMetadataStorage().unions.map<UnionTypeInfo>(
      unionMetadata => {
        ...
        const typesThunk = () => {
          unionObjectTypesInfo.push(
            ...unionMetadata
              ...
              .map(
                objectTypeCls =>
                  this.objectTypesInfo.find(type => type.target === objectTypeCls)!
              )
          )
    ...
    this.objectTypesInfo = getMetadataStorage().objectTypes.map<ObjectTypeInfo>(
      objectType => {
          ...
          type: new GraphQLObjectType({
            ...
            fields: () => {
              ...
              if (objectType.interfaceClasses) {
                const implementedInterfaces = getMetadataStorage().interfaceTypes.filter(
                  it => objectType.interfaceClasses!.includes(it.target)
                )
      ...
      interfaceType => {
        ...
        return {
          ...
          type: new GraphQLInterfaceType({
            ...
            astNode: getInterfaceTypeDefinitionNode(
              interfaceType.name,
              interfaceType.directives
            ),
            interfaces: () => {
              let interfaces = (
                interfaceType.interfaceClasses || []
              ).map<GraphQLInterfaceType>(
                interfaceClass =>
                  this.interfaceTypesInfo.find(info => info.target === interfaceClass)!
                    .type
              )
              // copy interfaces from super class
              if (hasExtended) {
            ...
            fields: () => {
              ...
              if (interfaceType.interfaceClasses) {
                const implementedInterfacesMetadata =
                  getMetadataStorage().interfaceTypes.filter(it =>
    ...
    this.inputTypesInfo = getMetadataStorage().inputTypes.map<InputObjectTypeInfo>(
      inputType => {
        ...
        return {
          ...
          type: new GraphQLInputObjectType({
            ...
            astNode: getInputObjectTypeDefinitionNode(
              inputType.name,
  ...
  private static buildRootQueryType(resolvers?: Function[]): GraphQLObjectType {
    const queriesHandlers = this.filterHandlersByResolvers(
  ...
  private static generateSubscriptionsFields<T = any, U = any>(
    ...
    return subscriptionsHandlers.reduce<GraphQLFieldConfigMap<T, U>>(
      (fields, handler) => {
        // omit emitting abstract resolver fields
        if (handler.resolverClassMetadata && handler.resolverClassMetadata.isAbstract) {
          return fields
        ...
        if (handler.subscribe) {
          ...
          if (typeof handler.topics === "function") {
            ...
            pubSubIterator = (payload, args, context, info) => {
              const resolverTopicData: ResolverTopicData = {
                payload,
                args,
                context,
                info,
              ...
              if (Array.isArray(topics) && topics.length === 0) {
                throw new MissingSubscriptionTopicsError(
                  handler.target,
                  handler.methodName
                )
              }
          ...
          subscribeFn = handler.filter
            ? withFilter(pubSubIterator, (payload, args, context, info) => {
                const resolverFilterData: ResolverFilterData = {
                  payload,
                  args,
                  context,
                  info,
  ...
  private static getGraphQLOutputType(
    ...
    if (!gqlType) {
      const unionType = this.unionTypesInfo.find(
        it => it.unionSymbol === (type as symbol)
      )
      if (unionType) {
  ...
  private static getResolveTypeFunction<TSource = any, TContext = any>(
    ...
    return async (...args) => {
      ...
      return possibleObjectTypesInfo.find(
        objectType => objectType.target === resolvedType
      )?.type.name
    }
  }
  ...
  private static filterHandlersByResolvers<T extends ResolverMetadata>(
    ...
    return resolvers
      ? handlers.filter(query => resolvers.includes(query.target))
      : handlers
  }
  ...
  private static filterTypesInfoByIsAbstractAndOrphanedTypesAndExtractType(
    ...
    return typesInfo
      .filter(
        it => !it.isAbstract && (!orphanedTypes || orphanedTypes.includes(it.target))
      )
      .map(it => it.type)
  }
}
